...
for (const environment of ['local', 'preview', 'production']) {
  ...
  const build = spawnSync(process.execPath, ['node_modules/astro/bin/astro.mjs', 'build', '--outDir', directory], {
    env: { ...process.env, NODE_ENV: 'production', DEPLOY_ENV: environment, SITE_URL: fixtureOrigin, MAINTAINER_NAME: 'Build test fixture', CONTACT_EMAIL: 'qa@acme.org', ASTRO_TELEMETRY_DISABLED: '1' },
    encoding: 'utf8',
  });
